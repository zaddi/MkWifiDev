/*  minimal.cpp - Example showing typical use of the MkWifiDev library
...
void setup() {
    ...
    DBG_VERBOSE("Starting MkWifiDev Minimal Demo");

    DBG_INFO("This demo includes 'over-the-air' firmware update support...");
    DBG_INFO("  ...and remote 'serial' terminal access!");

    DBG_DEBUG("About to connect to '%s' and set our device name to %s", ssid, devname);
...
void loop() {
  ...
  if(millis()-tprev > 5000)
    DBG_DEBUG("Press Ctrl-A to toggle mode. Running for %d sec", (tprev += 5000)/1000);

  // Check for user input (automatically uses terminal if connected otherwise serial port)
